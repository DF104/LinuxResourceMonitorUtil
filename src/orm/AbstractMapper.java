...
public abstract class AbstractMapper<T> {
	...
	static {
		try {
			props.load(AbstractMapper.class.getClassLoader()
				       .getResourceAsStream("mapper.properties"));
		...
		for (Field f:fields) {
			for (Method m:methods) {
				if (m.getName().equals("set" + f.getName().substring(0, 1).toUpperCase() + f.getName().substring(1))) {
					...
					String columnName = props.getProperty(clazz.getName().substring(clazz.getName().lastIndexOf(".") + 1) + "." + f.getName());
